...
from dataset_utils.ilab2m import ILAB2M
from dataset_utils.norb import smallNORB
from utils.config import *
from models.loss_functions import get_lossfn
from utils.device_setting import device
from utils.utils import plot_grad_flow_v2
import re
...
class EngineBase:
    def __init__(self, model, opt):
        ...
        if opt.DATA.NAME == "NVPD":
            ...
            self.train_loader = data.DataLoader(train_set, batch_size=self.opt.TRAIN.BATCH_SIZE,
                                                shuffle=self.opt.TRAIN.SHUFFLE, drop_last=True)
            self.test_loader = data.DataLoader(test_set, batch_size=self.opt.TEST.BATCH_SIZE,
                                               shuffle=self.opt.TEST.SHUFFLE, drop_last=True)
        elif opt.DATA.NAME == "ilab2m":
            train_dir = osp.join(opt.DATA.ROOT + "/train_img")
            test_dir = osp.join(opt.DATA.ROOT + "/test_img")
            train_set = ILAB2M(train_dir, opt.DATA.SIZE)
            test_set = ILAB2M(test_dir, opt.DATA.SIZE)
            self.train_loader = data.DataLoader(train_set, batch_size=self.opt.TRAIN.BATCH_SIZE,
                                                shuffle=self.opt.TRAIN.SHUFFLE, drop_last=True)
            self.test_loader = data.DataLoader(test_set, batch_size=self.opt.TEST.BATCH_SIZE,
                                               shuffle=self.opt.TEST.SHUFFLE, drop_last=True)

        elif re.match("SmallNORB_*", self.opt.DATA.NAME):

            mode = opt.DATA.NAME.split("_")[-1]
            path = osp.join(opt.DATA.ROOT + "/train_img")

            train_set = smallNORB(path, train=True, download=True, mode=mode)
            test_set = smallNORB(path, train=False, download=True, mode=mode)
            self.train_loader = data.DataLoader(train_set, batch_size=self.opt.TRAIN.BATCH_SIZE,
                                                shuffle=self.opt.TRAIN.SHUFFLE, drop_last=True)
            self.test_loader = data.DataLoader(test_set, batch_size=self.opt.TEST.BATCH_SIZE,
                                               shuffle=self.opt.TEST.SHUFFLE, drop_last=True)
    ...
    def optimizer_setup(self, params):
        ############  OPTIMIZER and LOSS SETUP ###########################
        self.loss_fn = get_lossfn(self.opt).to(device)
    ...
    def device_setup(self):
        raise NotImplementedError("Must override device_setup")

    def test_step(self, batch):
        raise NotImplementedError("Must override test_step")

    def train_step(self, batch):
        raise NotImplementedError("Must override train_step")
    ...
    def evaluate(self, eval_set):
        """Evaluates accuracy for given set of samples. Note that this is a basic accuracy evaluation"""

        print("Me evaluate undead skull!")
        self.model.eval()
        true_positive = torch.tensor(0)

        for batch in tqdm.tqdm(eval_set):
            true_positive += self.test_step(batch)

        accuracy = torch.FloatTensor([100]) * true_positive.float() / torch.FloatTensor([len(eval_set.dataset)])
        return accuracy


class EngineBasic(EngineBase):
    def __init__(self, model, opt):
        super(EngineBasic, self).__init__(model, opt)
        self.device_setup()
        params = list(self.model.parameters())
        self.device_setup()
        self.optimizer_setup(params)
    ...
    def train(self):
        ...
        while self.train_it < self.opt.TRAIN.MAX_IT:
            ...
            for batch in tqdm_for:
                ...
                if self.train_it != 0:
                    ...
                    if self.train_it % self.opt.TRAIN.LOG_INTERVAL == (self.opt.TRAIN.LOG_INTERVAL - 1):
                        ...
                        tqdm_for.set_description(info)
                        running_loss = 0.0
                        self.log(info)
                        self.wandb.log({"Current Batch Loss": loss.item()})

            accuracy = self.evaluate(self.test_loader)
            info = "Epoch: {} \tIteration: {}\t TEST ACCURACY: {}".format(self.epoch, self.train_it,
                                                                          accuracy)
            self.log(info)
            ...
            self.log(
                "Epoch: {} \tIteration: {} \tAvg. Epoch Loss: {}".format(self.epoch, self.train_it, avg_epoch_loss))
            self.epoch += 1
...
class EngineCap(EngineBasic):
    ...
    def test_step(self, batch):
        with torch.no_grad():
            x = batch[0]
            y = batch[1]
            y_hat = self.model(x.to(device))[1]
            return (torch.max(y_hat, dim=-1)[1] == y.to(device)).sum().item()
...
class EngineNovel(EngineBase):
    def __init__(self, model, decoder, opt):
        super(EngineNovel, self).__init__(model, opt)
        ...
        #     nn.Linear(512, 1024),
        #     nn.ReLU(inplace=True),
        #     nn.Linear(1024, (opt.DATA.SIZE ** 2)*opt.DATA.CHANNELS),
        #     nn.Sigmoid()
        # )

        self.decoder_model = decoder
        ########### ADD RECON NETWORK PARAMETERS TO NETWORK #############
        params = list(self.model.parameters())
    ...
    def train_step(self, batch):
        ...
        vec_mask = torch.eye(self.opt.DATA.NUM_CLASS, device=y_feat.device).index_select(dim=0,
                                                                                         index=y.squeeze()).unsqueeze(
            -1)
        reconstruct = self.decoder_model((vec_mask * y_feat).flatten(-2))
        reconstruction_loss = self.reconstruction_loss(x, reconstruct)
        ...
        if self.train_it % self.opt.TRAIN.LOG_INTERVAL == (self.opt.TRAIN.LOG_INTERVAL - 1):
            if not x.shape[0] < 3:
                tt = torch.cat(
                    [reconstruct[:10, :].reshape(-1, self.opt.DATA.CHANNELS, self.opt.DATA.SIZE, self.opt.DATA.SIZE),
                     x[:10]], dim=0)
    ...
    def test_step(self, batch):
        with torch.no_grad():
            ...
            return (torch.max(y_hat.squeeze(-1), dim=1)[1] == y.to(device).squeeze()).sum().item()
    ...
    def train(self):
        ...
        while self.train_it < self.opt.TRAIN.MAX_IT:
            ...
            tqdm_for = tqdm.tqdm(self.train_loader)
            for batch in tqdm_for:
                loss, ce_loss, reconstruct_loss = self.train_step(batch)

                tqdm_for.set_description(
                    "RECON LOSS: {:.4f}\tCE LOSS: {:.4f} \t LR: {}\t IT: {}".format(reconstruct_loss, ce_loss,
            ...
            if re.match("SmallNORB_*", self.opt.DATA.NAME):
                accuracy = self.evaluate(self.train_loader)
                info = "Epoch: {} \tIteration: {}\t TRAIN ACCURACY: {}".format(self.epoch, self.train_it,
                                                                              accuracy)
                self.log(info)
    ...
    def train(self):
        ...
        while self.train_it < self.opt.TRAIN.MAX_IT:
            ...
            for x, y, q in tqdm_for:
                ...
                tqdm_for.set_description(
                    "RECON LOSS: {:.4f}\tCE LOSS: {:.4f}\tNCE LOSS: {:.4f}\tLR: {}\tIT: {}".format(reconstruct_loss,
                                                                                                   ce_loss, nce_loss,
...
class EngineNovelNoFeat:
    def __init__(self, model, opt):
        ...
        print("Training Engine started with the following opt: \n{}".format(self.opt.dump()))
        self.model = model
        self.loss_fn = get_lossfn(opt).to(device)
        params = list(self.model.parameters())
        if self.opt.TRAIN.OPTIMIZER.NAME == "adam":
            self.optimizer = torch.optim.Adam(params, lr=self.opt.TRAIN.OPTIMIZER.LR)
        else:
